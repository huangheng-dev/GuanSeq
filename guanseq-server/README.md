...
- 周期任务自动生成、备件领用、维修成本、设备采集、网关、点位、自动报警、OEE 和远程控制尚未接入，不把人工执行记录宣称为实时遥测。
...
└─ equipment     设备台账、人工状态机、点检保养、维修验收与不可变事件
```
...
第一条制造主闭环已经形成真实证据链，设备台账、点检/保养和维修验收事实也已落地。下一阶段优先补齐不依赖物联网基础设施的设备运维成本证据：

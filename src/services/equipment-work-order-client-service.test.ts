...
import { EquipmentWorkOrderClientError, loadEquipmentWorkOrderDetail, refreshEquipmentWorkOrders, submitEquipmentMaintenanceCostMutation, submitEquipmentWorkOrderMutation } from "./equipment-work-order-client-service";
...
const workOrder: EquipmentWorkOrder = { id: "b1000000-0000-4000-8000-000000000001", workOrderNumber: "WO-20260825-0001", workType: "REPAIR", sourceType: "BREAKDOWN", sourceWorkOrderId: null, sourcePlanId: null, sourceDueDate: null, assetId: asset.id, assetCode: asset.assetCode, assetName: asset.assetName, assetLocation: asset.location, assetOperatingStatus: "DOWN", assetVersion: 1, title: "故障维修", description: "排查设备故障", priority: "HIGH", status: "PLANNED", plannedStartAt: "2026-08-25T02:00:00Z", dueAt: "2026-08-25T10:00:00Z", assignee: "周凯", outcome: null, completionNotes: null, startedAt: null, submittedAt: null, completedAt: null, version: 0, createdAt: "2026-08-25T01:00:00Z", updatedAt: "2026-08-25T01:00:00Z", costEvidence: null, availableActions: ["START", "CANCEL"], events: [] };
...
describe("equipment work order client service", () => {
  ...
  it("submits spare issue with the repair work order version and parses cost evidence", async () => {
    const result = { workOrderVersion: 2, costEvidence: { spareCost: 400, laborCost: 0, totalCost: 400, currency: "CNY",
      basis: "运维估算", spareTransactions: [], laborTransactions: [], availableActions: ["ISSUE_SPARE"] } };
    const fetchMock = vi.fn().mockResolvedValue(Response.json({ result })); vi.stubGlobal("fetch", fetchMock);
    await expect(submitEquipmentMaintenanceCostMutation({ operation: "issueSpare", id: workOrder.id,
      sparePartId: "c1000000-0000-4000-8000-000000000001", warehouseId: "71000000-0000-4000-8000-000000000001",
      quantity: 5, reason: "更换故障轴承并保留证据", expectedVersion: 1 })).resolves.toEqual(result);
    expect(JSON.parse(fetchMock.mock.calls[0][1].body)).toEqual(expect.objectContaining({ operation: "issueSpare", expectedVersion: 1, quantity: 5 }));
  });
});

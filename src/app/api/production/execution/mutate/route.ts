...
const mutation = z.discriminatedUnion("operation", [
  z.object({ operation: z.literal("report"), orderId: z.string().uuid(), quantity: z.number().positive(), shiftName: z.string().min(1).max(80), operatorName: z.string().min(1).max(80), note: z.string().max(500).nullable(), expectedOrderVersion: z.number().int().nonnegative() }),

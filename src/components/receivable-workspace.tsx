...
import { submitCreateReceivableInvoice, submitPostReceivableReceipt } from "@/services/receivable-client-service";
import type { ReceivablePageData } from "@/services/receivable-server-service";
import { MaterialIcon } from "./material-icon";
import { RoundedSelect } from "./rounded-select";
const statusLabels = { OPEN: "待收款", PARTIALLY_PAID: "部分收款", PAID: "已收清" } as const;
...
function tone(status: ReceivableInvoiceRecord["status"]) { return status === "PAID" ? "good" : status === "PARTIALLY_PAID" ? "info" : "warn"; }
...
function InvoiceDrawer({ invoice, onClose, onReceipt }: {
    invoice: ReceivableInvoiceRecord;
    onClose: () => void;
    onReceipt: () => void;
}) {
    return <GsDrawerHost onClose={onClose}><aside className="businessDrawer" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><header className="recordDrawerHeader"><div><h2>{invoice.invoiceNumber}</h2><p>{invoice.customerName} · {invoice.orderNumber}</p></div><GsButton className="iconButton" aria-label="关闭详情" onClick={onClose} htmlType="submit"><MaterialIcon name="close" size={20}/></GsButton></header><div className="recordDrawerBody"><section className="detailSummary"><div><small>含税金额</small><strong>{money(invoice.grossAmount, invoice.currency)}</strong></div><div><small>已收 / 待收</small><strong>{money(invoice.receivedAmount, invoice.currency)} / {money(invoice.outstandingAmount, invoice.currency)}</strong></div><div><small>账期</small><strong>{invoice.invoiceDate} → {invoice.dueDate}</strong></div></section><section className="drawerSection"><h3>开票明细</h3>{invoice.lines.map((line) => <article className="receivableDetailLine" key={line.id}><span><strong>{line.materialCode} · {line.materialName}</strong><small>{line.invoiceQuantity} {line.unit} × {money(line.unitPrice, invoice.currency)}</small></span><b>{money(line.grossAmount, invoice.currency)}</b></article>)}</section><section className="drawerSection"><h3>收款证据</h3>{invoice.receipts.length ? invoice.receipts.map((receipt) => <article className="receivableDetailLine" key={receipt.id}><span><strong>{receipt.receiptNumber}</strong><small>{receipt.receiptDate} · {paymentLabels[receipt.paymentMethod]}</small></span><b>{money(receipt.amount, invoice.currency)}</b></article>) : <p className="drawerEmptyText">尚无收款记录。</p>}</section></div><footer className="recordDrawerFooter"><GsButton className="secondaryButton" onClick={onClose} htmlType="submit">关闭</GsButton>{invoice.status !== "PAID" ? <GsButton className="primaryButton" onClick={onReceipt} htmlType="submit"><MaterialIcon name="payments" size={17}/>登记收款</GsButton> : null}</footer></aside></GsDrawerHost>;
}
export function ReceivableWorkspace({ initialData, pathname }: {
    ...
    const overdue = invoices.filter((invoice) => invoice.status !== "PAID" && invoice.dueDate < today()).length;
    ...
    function replace(invoice: ReceivableInvoiceRecord) { setInvoices((current) => current.map((item) => item.id === invoice.id ? invoice : item)); setDetail(invoice); setReceiving(null); setToast(`${invoice.invoiceNumber} 收款已登记`); window.setTimeout(() => setToast(""), 2800); }
    ...
    </section><div className="ledgerInsight"><MaterialIcon name="fact_check" size={18}/>当前闭环覆盖正向开票与收款核销；红字发票、退款、反核销、总账凭证和税务平台对接尚未接入，不在本阶段冒充完成。</div>
    {creatingFor !== null ? <InvoiceDialog orders={orders} initialOrderId={creatingFor || undefined} onClose={() => setCreatingFor(null)} onSaved={(invoice) => saved(invoice, `${invoice.invoiceNumber} 已开具`)}/> : null}{receiving ? <ReceiptDialog invoice={receiving} onClose={() => setReceiving(null)} onSaved={replace}/> : null}{detail ? <InvoiceDrawer invoice={detail} onClose={() => setDetail(null)} onReceipt={() => { setReceiving(detail); setDetail(null); }}/> : null}{toast ? <div className="toastMessage" role="status"><MaterialIcon name="check_circle" filled size={18}/>{toast}</div> : null}</div>;
}

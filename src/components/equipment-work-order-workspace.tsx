...
import { EquipmentMaintenancePlanSection } from "./equipment-maintenance-plan-section";
...
const sourceLabels = { MANUAL: "人工计划", BREAKDOWN: "故障报修", INSPECTION_FAILURE: "点检异常", MAINTENANCE_FAILURE: "保养异常", MAINTENANCE_PLAN: "周期模板" } as const;
...
function WorkOrderCreateDialog({ workType, assets, onClose, onSaved }: { workType: EquipmentWorkType; assets: EquipmentAsset[]; onClose: () => void; onSaved: (order: EquipmentWorkOrder) => void }) {
  ...
  return <GsModalHost onClose={() => { if (!pending) onClose(); }}><section className="businessDialog equipmentWorkOrderDialog" role="dialog" aria-modal="true" aria-labelledby="equipment-work-order-form-title" onMouseDown={(event) => event.stopPropagation()}>
    <header className="dialogHeader"><span className="dialogTitleMark"><MaterialIcon name={iconFor(workType)} size={22}/></span><div><h2 id="equipment-work-order-form-title">新建{typeLabels[workType]}</h2><p>创建一次性任务；周期作业请使用保养计划中的周期模板。</p></div><GsButton className="iconButton" onClick={onClose} disabled={pending} aria-label="关闭运维工单表单" htmlType="button"><MaterialIcon name="close"/></GsButton></header>
    ...
    </div>{error ? <div className="formError" role="alert">{error}</div> : null}<footer className="dialogFooter"><span><MaterialIcon name="event" size={16}/>一次性任务与周期模板来源分开留证</span><div><GsButton onClick={onClose} disabled={pending} htmlType="button">取消</GsButton><GsButton intent="primary" loading={pending} htmlType="submit">保存计划</GsButton></div></footer></form> : <div className="businessEmptyState"><MaterialIcon name="block" size={28}/><strong>没有可创建此类工单的设备</strong><p>{workType === "REPAIR" ? "请先从设备台账报告故障，系统会自动生成维修工单。" : "当前工作区没有可用设备。"}</p><GsButton onClick={onClose} htmlType="button">关闭</GsButton></div>}
  </section></GsModalHost>;
}
...
function WorkOrderDrawer({ order, canMaintain, loading, error, onClose, onAction, onCostAction }: { order: EquipmentWorkOrder; canMaintain: boolean; loading: boolean; error: string; onClose: () => void; onAction: (action: EquipmentWorkOrderAction) => void; onCostAction: (action: CostAction) => void }) {
  return <GsDrawer open title={`${order.workOrderNumber} · ${order.title}`} onClose={onClose} className="equipmentAssetDrawer"><div className="equipmentDetailStatus"><em className={`businessStatus businessStatus${statusTones[order.status]}`}>{statusLabels[order.status]}</em><span>{typeLabels[order.workType]}</span><small>设备状态：{assetStatusLabels[order.assetOperatingStatus]} · 版本 {order.assetVersion}</small></div>
    <dl className="detailLedger"><div><dt>设备</dt><dd>{order.assetName} · {order.assetCode}</dd></div><div><dt>位置</dt><dd>{order.assetLocation}</dd></div><div><dt>来源</dt><dd>{sourceLabels[order.sourceType]}{order.sourceDueDate ? ` · 到期 ${order.sourceDueDate}` : ""}</dd></div><div><dt>优先级</dt><dd>{priorityLabels[order.priority]}</dd></div><div><dt>责任人</dt><dd>{order.assignee}</dd></div><div><dt>计划开始</dt><dd>{dateText(order.plannedStartAt)}</dd></div><div><dt>要求完成</dt><dd>{dateText(order.dueAt)}</dd></div><div><dt>执行结论</dt><dd>{order.outcome ? (order.outcome === "PASS" ? "通过" : "不通过") : "待提交"}</dd></div></dl>
...
export function EquipmentWorkOrderWorkspace({ initialData, view }: { initialData: EquipmentWorkOrderPageData; view: "inspections" | "maintenance" | "work-orders" }) {
  ...
  return <div className="businessPage equipmentWorkOrderPage"><header className="pageHeading businessPageHeading"><div className="pageTitleGroup"><span className="pageTitleIcon"><MaterialIcon name={iconFor(workType)} size={23}/></span><div><h2>{title}</h2><p>{workType === "INSPECTION" ? "计划现场点检，异常自动转入维修闭环。" : workType === "PREVENTIVE_MAINTENANCE" ? "按停机窗口执行保养并记录结果。" : "从故障报修推进到维修完工与现场验收。"}</p></div></div><div className="pageHeadingActions"><GsButton onClick={refresh} loading={refreshing} htmlType="button"><MaterialIcon name="refresh" size={17}/>刷新</GsButton>{pageData.canMaintain ? <GsButton intent="primary" onClick={() => setCreateOpen(true)} htmlType="button"><MaterialIcon name="add" size={17}/>新建{typeLabels[workType]}</GsButton> : null}</div></header>
    <section className="equipmentTruthBanner"><MaterialIcon name="person_edit" size={21}/><div><strong>人工执行与受控生成</strong><p>工单、设备状态、周期模板与生成批次真实落库；首版由人员触发生成，独立调度、设备采集、报警和 OEE 尚未接入。</p></div><span>后台调度未接入</span></section>
    {view === "maintenance" ? <EquipmentMaintenancePlanSection initialData={initialData.maintenancePlans} assets={assets} onGenerated={refresh}/> : null}
    ...
    <section className="businessLedger"><div className="businessToolbar"><div className="businessSearch"><MaterialIcon name="search" size={18}/><GsInput aria-label={`搜索${title}`} value={query} placeholder="搜索工单号、设备、标题或责任人" onChange={(event) => { setQuery(event.target.value); setPage(1); }}/></div><div className="equipmentFilters"><RoundedSelect ariaLabel="工单状态" options={["全部状态", ...Object.values(statusLabels)]} value={status} onValueChange={(value) => { setStatus(value); setPage(1); }}/></div></div>
      <div className="equipmentWorkOrderTable" role="table" aria-label={`${title}列表`}><div className="equipmentWorkOrderTableHeader" role="row"><span>工单 / 任务</span><span>设备 / 来源</span><span>状态 / 优先级</span><span>计划 / 责任</span><span>操作</span></div>{rows.length ? rows.map((order) => <div className="equipmentWorkOrderTableRow" role="row" key={order.id}><span><strong>{order.title}</strong><small>{order.workOrderNumber} · {typeLabels[order.workType]}</small></span><span><strong>{order.assetName}</strong><small>{order.assetCode} · {sourceLabels[order.sourceType]}{order.sourceDueDate ? ` · 到期 ${order.sourceDueDate}` : ""}</small></span><span><em className={`businessStatus businessStatus${statusTones[order.status]}`}>{statusLabels[order.status]}</em><small>{priorityLabels[order.priority]}优先级 · 设备{assetStatusLabels[order.assetOperatingStatus]}</small></span><span><strong>{dateText(order.dueAt)}</strong><small>{order.assignee}</small></span><span className="businessRowActions"><GsButton onClick={() => openDetail(order)} aria-label={`查看${order.workOrderNumber}`} htmlType="button"><MaterialIcon name="visibility" size={18}/></GsButton>{pageData.canMaintain && order.availableActions[0] ? <GsButton onClick={() => { setSelected(order); setAction(order.availableActions[0]); }} aria-label={`处理${order.workOrderNumber}`} htmlType="button"><MaterialIcon name="published_with_changes" size={18}/></GsButton> : null}</span></div>) : <div className="businessEmptyState"><MaterialIcon name={iconFor(workType)} size={28}/><strong>没有符合条件的{typeLabels[workType]}</strong><p>{pageData.canMaintain ? "可新建一次性任务；保养页面也可从周期模板生成到期任务。" : "当前工作区暂时没有可见任务。"}</p>{pageData.canMaintain ? <GsButton intent="primary" onClick={() => setCreateOpen(true)} htmlType="button">新建{typeLabels[workType]}</GsButton> : null}</div>}</div>

...
function actionsFor(status: EquipmentOperatingStatus): EquipmentAssetAction[] {
  if (status === "IDLE") return ["START", "REPORT_BREAKDOWN", "START_MAINTENANCE", "INACTIVATE"];
  if (status === "RUNNING") return ["STOP", "REPORT_BREAKDOWN"];
  if (status === "DOWN") return ["START_MAINTENANCE"];
  if (status === "MAINTENANCE") return ["COMPLETE_MAINTENANCE"];
  return [];
}

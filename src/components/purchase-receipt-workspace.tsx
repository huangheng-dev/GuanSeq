...
function ReceiptForm({ data, onClose, onSaved }: {
    ...
    async function submit(event: FormEvent) {
        ...
        setPending(true);
        try {
            onSaved(await submitCreatePurchaseReceipt({ purchaseOrderId: order.id, warehouseId, locationId, lines }));
        }
        catch (reason) {

...
export function encodeCode128B(value: string) {
  if (!value.length || [...value].some((character) => character.charCodeAt(0) < 32 || character.charCodeAt(0) > 126)) {
    throw new Error("Code 128B 仅支持非空 ASCII 32–126 标签载荷");
  }
  ...
  return { bars, checksum, width: cursor + quietZone };
}

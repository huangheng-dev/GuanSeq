...
describe("backend page registry", () => {
  it("keeps every formal backend route unique and explicitly registered", () => {
    ...
    expect(getBackendPageDataKey("/settings/organization/structure")).toBe("organizationStructure");
  ...
  it("fails closed when formal data is absent or marked unavailable", () => {
    ...
    expect(isBackendPageUnavailable("/settings/organization/structure", {})).toBe(true);
    expect(isBackendPageUnavailable("/settings/organization/structure", { organizationStructure: { source: "backend" } })).toBe(false);
